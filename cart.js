...
function setupCartPage() {
    ...
    // Adicionar ações do carrinho (se ainda não existirem)
    if (!document.querySelector(".cart-actions")) {
        const cartActions = document.createElement("div");
        cartActions.className = "cart-actions";
        cartActions.innerHTML = `
            <a href="/Kayblakk-Bebidas/pages/categories.html" class="continue-shopping">
    ...
    // Evento para atualização da taxa de entrega
    if (deliveryFeeInput) {
        deliveryFeeInput.addEventListener("input", updateOrderSummary);
    }
}
...
function updateOrderSummary() {
    ...
    if (subtotalElement && totalElement && deliveryFeeInput) {
...
function setupCartEvents() {
    ...
    if (checkoutBtn) {
        checkoutBtn.addEventListener("click", function() {
            ...
            const whatsappNumber = "5500123456789"; // Substitua pelo número real
...
// Função de notificação (assumindo que está em main.js ou globalmente disponível)
function showNotification(message, type) {
    console.log(`Notificação (${type}): ${message}`);
    // Implementação real da notificação (ex: exibir um toast na tela)
    // Você pode adicionar um elemento div no HTML e manipulá-lo aqui
